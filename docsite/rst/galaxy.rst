...
You can set custom values for the variables that are injected into the role skeleton templates by creating a yaml file that sets the values you care about, and setting that path in ansible.cfg.  Alternatively the init vars file path can be set in the 'ANSIBLE_GALAXY_INIT_VARS_FILE' environment variable.
